...
export default function WorkshopPage() {
  ...
  // Mock data for workshops (100 items)
  const workshopsData = Array.from({ length: 100 }, (_, index) => {
    const workshopTypes = ['Technical Skills', 'Soft Skills', 'Leadership', 'Communication', 'Problem Solving']
    const statuses = ['UPCOMING', 'ONGOING', 'COMPLETED']
    const workshopType = workshopTypes[index % workshopTypes.length]
    ...
    return {
      ...
      participants: 20 + (index % 30), // 20-49 participants
      status: status
    }
  })
  ...
  const filteredWorkshops = workshopsData.filter(workshop => {
    ...
    // Apply filters
    const matchesType = !filters.workshopType || workshop.type === filters.workshopType
    const matchesStatus = !filters.status || workshop.status === filters.status
    const matchesDate = !filters.date || workshop.date === filters.date
  ...
  const currentData = filteredWorkshops.slice(startIndex, endIndex)
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-900">Workshop Title</th>
                  ...
                  <tr key={workshop.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <input
                        ...
                        onChange={() => handleSelectItem(workshop.id)}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                    </td>
                    <td className="py-3 px-4 text-gray-900">{workshop.id}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.title}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.type}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.instructor}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.date}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.time}</td>
                    <td className="py-3 px-4 text-gray-900">{workshop.participants}</td>
                    <td className="py-3 px-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusBadge(workshop.status)}`}>
                        {workshop.status}
                      </span>
                    </td>
                    <td className="py-3 px-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFilters({...filters, workshopType: e.target.value})}
