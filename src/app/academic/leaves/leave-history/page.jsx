...
export default function LeaveHistoryPage() {
  ...
  // Mock data for leave history (100 items)
  const leaveHistoryData = Array.from({ length: 100 }, (_, index) => {
    const leaveTypes = ['Casual Leave', 'Medical Leave', 'Special Leave']
    const statuses = ['PENDING', 'APPROVED']
    const leaveType = leaveTypes[index % leaveTypes.length]
    const status = statuses[index % statuses.length]
    const days = 1 + (index % 5) // 1-5 days
    
    return {
      id: `L${String(index + 1).padStart(3, '0')}`,
      leaveType: leaveType,
      leaveDate: '01 Jan 2025 - 01 Jan 2025',
      days: days,
  ...
  const filteredLeaves = leaveHistoryData.filter(item =>
  ...
  const handleApplyLeave = () => {
    router.push('/academic/leaves/leave-history/apply')
  }
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h3 className="font-semibold text-gray-900 text-sm">{item.leaveType}</h3>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-gray-900 font-medium">{item.leaveDate}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">No. of Days:</span>
          <span className="text-gray-900 font-medium">{item.days}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">Applied On:</span>
          <span className="text-gray-900 font-medium">{item.appliedOn}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="mt-6">
        ...
        <div className="space-y-3">
          {currentData.map((item) => (
            <MobileLeaveCard key={item.id} item={item} />
          ))}
        </div>
  ...
  return (
    <PageLayout rightContent={
      ...
      <div className="w-full">
        {/* Leaves History List Section */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-4 md:p-6">
            {/* Header with title and controls */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <h2 className="text-lg md:text-xl font-bold text-gray-900">Leaves History List</h2>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-gray-900">Leave Date</th>
                    <th className="text-left py-3 px-4 font-semibold text-gray-900">No. of Days</th>
                    <th className="text-left py-3 px-4 font-semibold text-gray-900">Applied On</th>
                ...
                <tbody>
                  {currentData.map((item) => (
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-gray-900">{item.leaveType}</td>
                      <td className="py-3 px-4 text-gray-900">{item.leaveDate}</td>
                      <td className="py-3 px-4 text-gray-900">{item.days}</td>
                      <td className="py-3 px-4 text-gray-900">{item.appliedOn}</td>
