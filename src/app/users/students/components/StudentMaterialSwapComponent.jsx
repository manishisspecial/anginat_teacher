...
const StudentMaterialSwapComponent = () => {
  ...
  const ActionLinks = ({ id }) => (
    ...
    <span 
      ...
      onClick={(e) => {
        e.stopPropagation();            
        router.push(`/users/students/view-students/${id}`)
      }}
    >
      View
    </span>
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3 shadow-sm">
      <div className="flex justify-between items-start mb-3">
        ...
        <span className="text-blue-600 text-sm cursor-pointer hover:underline"
              onClick={() => router.push(`/users/students/view-students/${student.id}`)}>
          View
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <span className="ml-2 text-gray-900">{student.admissionNo}</span>
        </div>
        <div>
          <span className="text-gray-500">Roll No:</span>
          <span className="ml-2 text-gray-900">{student.rollNo}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="ml-2 text-gray-900">{student.section}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="ml-2 text-gray-900">{student.dateOfJoining}</span>
        </div>
        <div className="col-span-2">
          <span className="text-gray-500">DOB:</span>
          <span className="ml-2 text-gray-900">{student.dob}</span>
        </div>
      </div>
    </div>
  );
...
export default StudentMaterialSwapComponent;